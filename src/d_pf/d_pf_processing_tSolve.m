...
[tSol_DeePC1,tSol_CLDeePC1,tSol_Oracle1] = deal(nan(n_quant,num_n)); % quantiles w/ median of cost
[tSol_DeePC2,tSol_CLDeePC2,tSol_Oracle2] = deal(nan(1,num_n));       % mean cost
...
%     costs = cellfun(@(x) mean(x(Nbar+1:end)),a.results.Cost);
    len_x = length(a.results.y_CL{1});
    tSol = cell2mat(cellfun(@(x) x.',a.results.tSolves,'UniformOutput',false));
    tSol = 1e3*tSol; % [s] -> [ms]
    tSol_DeePC1(:,kn)   = quantile(tSol(:,1),quantiles);
    tSol_CLDeePC1(:,kn) = quantile(tSol(:,2),quantiles);
    tSol_DeePC2(:,kn)   = mean(tSol(:,1));
    tSol_CLDeePC2(:,kn) = mean(tSol(:,2));
    if num_c > 2
        tSol_Oracle1(:,kn)  = quantile(tSol(:,3),quantiles);
        tSol_Oracle2(:,kn)  = mean(tSol(:,3));
    end
end
%%
...
for idx = 1:idx_median-1
    if num_c > 2
        ff0{idx}=fill([pf_all fliplr(pf_all)],[tSol_Oracle1(idx,:),  fliplr(tSol_Oracle1(end-idx+1,:))],color{3},HandleVisibility='off');
        ff0{idx}.EdgeAlpha = 0.1; ff0{idx}.FaceAlpha = FaceAlpha; hold on;
    end
    ff1{idx}=fill([pf_all fliplr(pf_all)],[tSol_DeePC1(idx,:),  fliplr(tSol_DeePC1(end-idx+1,:))],  color{1},HandleVisibility='off');
    ff1{idx}.EdgeAlpha = 0.1; ff1{idx}.FaceAlpha = FaceAlpha; hold on;
    ff2{idx}=fill([pf_all fliplr(pf_all)],[tSol_CLDeePC1(idx,:),fliplr(tSol_CLDeePC1(end-idx+1,:))],color{2},HandleVisibility='off');
    ff2{idx}.EdgeAlpha = 0.1; ff2{idx}.FaceAlpha = FaceAlpha; hold on;
end
if num_c > 2
    l0 = plot(pf_all,tSol_Oracle1(idx_median,:),  Color=color{3},LineWidth=1.5); hold on;
    l01= plot(pf_all,tSol_Oracle2,Color=color{3}, LineStyle='--',LineWidth=1.5);
end
l1 = plot(pf_all,tSol_DeePC1(idx_median,:),   Color=color{1},LineWidth=1.5); hold on;
l11= plot(pf_all,tSol_DeePC2,Color=color{1},  LineStyle='--',LineWidth=1.5);
l2 = plot(pf_all,tSol_CLDeePC1(idx_median,:), Color=color{2},LineWidth=1.5);
l21= plot(pf_all,tSol_CLDeePC2,Color=color{2},LineStyle='--',LineWidth=1.5);
...
ylabel('computation time [ms]','Interpreter','latex','FontSize',13);
