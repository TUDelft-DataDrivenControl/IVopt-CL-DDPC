function [opts, u0, y0, xcl0, Z, Lf, Cz, Tcl, u_cl, y_cl] ... 
...
% ============================ create IVs =================================
[Z,nlcf]= get_Z(u0,y0,yr0,Cz0,Uf_iv2,opts); % Z is IV_4_DDPC object containing IVs

%% Get Subspace Predictive Controllers
% ---------------------- get (CL-)SPC controllers -------------------------
fprintf('Obtaining controllers...\n');

[Lf,Cz,x1_0_SPC,sigs] = get_Lf_Cz(Z,plant,u0,y0,ur1,yr1,Q,R,dR,opts,sigs);
...
% run simulations
for kCz = 1:nCz
    Czn = opts.Cases{kCz}; % controller name
